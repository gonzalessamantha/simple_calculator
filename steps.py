...
class CalculatorSteps(tk.Frame):
    ...
    def create_step_widget(self):
        ...
        title.grid(column=0, row=0, columnspan=4, pady=10)

        # Step Display
        self.step_display = tk.Label(self, text="Choose Operation", font=("Arial", 16, "bold"), bg='lightblue',
                                     relief='ridge')
    ...
    def hide_all_step_widgets(self):
        widgets = [self.number_1_label, self.number_1_entry, self.number_2_label,
                   self.number_2_entry, self.next_button, self.calculate_button,
                   self.result_label, self.try_again_button, self.exit_button]
    ...
    def calculate(self):
        try:
            ...
            self.hide_all_step_widgets()
            self.result_label.config(text=f"RESULT: {self.result:.3f}")
            self.result_label.grid(column=0, row=7, columnspan=4, pady=20)
